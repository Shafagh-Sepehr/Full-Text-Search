...
internal static partial class AppSettings
{
    ...
    internal static partial class RegexPatterns
    {
        ...
        [GeneratedRegex(@"(?<!\S)(?:(?:https?|ftp):\/\/|www\.|(?:\d{1,3}\.){3}\d{1,3}(?::\d{1,5})?\/|localhost(?::\d{1,5})?|[a-zA-Z0-9-]+\.[a-zA-Z]{2,}(?::\d{1,5})?)(?:[^\s()<>]+(?:$[\w\d]+$|([^[:punct:]\s]|\/))*)(?=\s|$)")]
        ...
        [GeneratedRegex(@"\s*(?:\+?\d{1,3}[-.\s]?)?(?:$\d{1,4}$[-\s]?|\d{1,4}[-.\s]?)?\d{1,4}[-.\s]?\d{1,4}[-.\s]?\d{1,9}(?:\s?(?:ext|x|extension|#)?\.?\s?\d{1,5})?\s*")]
